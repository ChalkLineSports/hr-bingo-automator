---
name: mlb-hr-derby-lineup-saturday
description: Generates Saturday night HR Derby lineup — runs Friday at noon CT
---

You are running the MLB HR Derby lineup generation job for Saturday night's games.

**Goal**: Generate a ready-to-upload Chalkline CSV of the top 32 likely home run hitters for TOMORROW's (Saturday) evening MLB games, post it to Slack, and log the run.
...
`:warning: *No HR Derby tomorrow (Sat)* — No MLB evening games (6 PM CT+) found for [Day, Mon DD]. Slate not generated.`
...
echo "$(TZ='America/Chicago' date '+%Y-%m-%dT%H:%M:%S%z') | lineup-saturday | target=YYYY-MM-DD | SUCCESS | N players | N games | estimated=true/false | warnings=N" >> /Users/joekustelski/.claude/logs/hr-derby.log
```
...
```bash
echo "$(TZ='America/Chicago' date '+%Y-%m-%dT%H:%M:%S%z') | lineup-saturday | target=YYYY-MM-DD | NO_GAMES" >> /Users/joekustelski/.claude/logs/hr-derby.log
```
...
:calendar: *Heads up — thin MLB slates ahead:*
  • Tue Apr 14: only 2 evening games — HR Derby may not run
```
